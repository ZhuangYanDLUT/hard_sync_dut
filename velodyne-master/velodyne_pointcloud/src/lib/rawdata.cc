...
namespace velodyne_rawdata
{
   ...
   */
  double RawData::unpack(const velodyne_msgs::VelodynePacket &pkt,
                       VPointCloud &pc)
  {
   ...
   *  @param pc shared pointer to point cloud (points are appended)
   */
  double RawData::unpack_vlp16(const velodyne_msgs::VelodynePacket &pkt,
                             VPointCloud &pc)
  {
    float azimuth;
    float azimuth_diff;
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
      ...
      if (UPPER_BANK != raw->blocks[block].header) {
        ...
        return 0.0;                         // bad packet: skip the rest
      }
	...
	//获取Lidar传递过来的时间戳
	uint32_t tmpt;
	tmpt = raw->timestamp[3];
	uint32_t the_time1 = tmpt<<24;
	tmpt = raw->timestamp[2];
	uint32_t the_time2 = tmpt<<16;
	tmpt = raw->timestamp[1];
	uint32_t the_time3 = tmpt<<8;
	tmpt = raw->timestamp[0];
	uint32_t the_time4 = tmpt;
	uint32_t the_time = the_time1 + the_time2 + the_time3 + the_time4;

	return the_time*1e-6;
  }  
